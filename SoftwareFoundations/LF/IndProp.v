...
Proof.
  intros.
  induction H as [| m IHn IHm].
  (* wjy induction as this? *)
  - apply le_n.
  - apply le_S in IHm. apply IHm.
Qed.

Lemma le_S_implies: forall m n, S m <= n -> m <= n.
Proof.
  intros.
  induction H.
...
Proof.
  ...
  - intros. apply Lno.
  - intros. apply IHLmn.
    apply le_S_implies in Lno. apply Lno.
Qed.
...
Proof.
  ...
  - right. apply le_n.
  - unfold lt in IHm. destruct IHm.
    + left. unfold lt. apply le_S in H. apply H.
    + inversion H. left. apply le_n. 
  - unfold lt in IHn. unfold lt. destruct IHn.
    + inversion H. right. apply le_n. left. apply n_le_m__Sn_le_Sm. apply H0.
    + right. apply le_S in H. apply H.
Qed.
...
Proof.
  intros n1 n2 m H.
  assert (H1: n1 <= n1 + n2). apply le_plus_l.
  assert (H2: n2 <= n1 + n2). rewrite add_comm. apply le_plus_l.
  split.
  - apply le_trans with (n:= n1 + n2). apply H1. apply H.
  - apply le_trans with (n:= n1 + n2). apply H2. apply H.
Qed.
...
Proof.
  intros. induction p.
  simpl. apply H.
  apply n_le_m__Sn_le_Sm in IHp. simpl in IHp. simpl. apply IHp.
Qed.
...
Proof.
  intros. induction p.
  - simpl. rewrite add_0_r. rewrite add_0_r. apply H.
  - apply n_le_m__Sn_le_Sm in IHp.
    rewrite plus_n_Sm in IHp. rewrite plus_n_Sm in IHp.
    apply IHp.
Qed.

Lemma le_adder: forall m n p, m + p <= n + p -> m <= n.
Proof.
  intros m n p. induction p.
  - rewrite add_0_r. rewrite add_0_r. intros. apply H.
  - intros. rewrite add_comm in H. rewrite ( add_comm n) in H. 
    simpl in H. apply Sn_le_Sm__n_le_m in H.
    rewrite add_comm in H. rewrite (add_comm _ n) in H.
    apply IHp in H. apply H.
Qed.
...
Proof.
  ...
  - intros. assert (H':= H). simpl in H. apply le_S_implies in H.
    apply IHn in H. destruct H. inversion H.
    + right. rewrite H0 in H'. assert (Hq: S m <= q). 
      simpl in H'. rewrite plus_n_Sm in H'.
      rewrite add_comm in H'. rewrite (add_comm p q) in H'.
      apply le_adder in H'. apply H'.
      apply le_S_implies in Hq. apply Hq.
...
Theorem n_lt_m__n_le_m : forall n m,
  ...
  n <= m.
Proof.
  intros. unfold lt in H. apply le_S_implies. apply H.
Qed.
...
Proof.
  ...
  - apply le_S_implies in H0. apply plus_le in H0. destruct H0. split.
...
Lemma leb_S_implies: forall m n, (m <=? n) = true -> (m <=? S n) = true.
Proof.
  intros m. induction m.
  - intros. simpl. reflexivity.
  - simpl. destruct n eqn: eqN.
    + discriminate.
    + intros. apply IHm in H. apply H.
Qed.
...
Proof.
  ...
  - intros. inversion H.
    ...
    + apply IHm in H1. apply leb_S_implies in H1. apply H1.
Qed.
...
Proof.
  ...
  - intros. induction H.
    + reflexivity.
    + simpl. apply eq_implies_succ_equal. apply IHR.
    + rewrite add_comm. simpl. apply eq_implies_succ_equal.
      rewrite add_comm. apply IHR.
    + simpl in IHR. rewrite add_comm in IHR. simpl in IHR.
      apply S_injective. apply S_injective. rewrite add_comm. apply IHR.
    + rewrite add_comm. apply IHR.
  - intros. generalize dependent o. induction m. induction n.
    + intros. simpl in H. rewrite <- H. apply c1.
    + intros. inversion H. simpl. apply c3. apply IHn. reflexivity.
    + intros. inversion H. simpl. apply c2. apply IHm. reflexivity.
Qed.
(** [] *)
...
Inductive subseq : list nat -> list nat -> Prop :=
  | subnil l: subseq [] l
  | subboth l1 l2 (x: nat) (H: subseq l1 l2): subseq (x::l1) (x::l2)
  | subext  l1 l2 (x: nat) (H: subseq l1 l2): subseq l1 (x::l2)
.
(* FILL IN HERE *)
...
Proof.
  intros. induction H.
  - apply subnil.
  - simpl. apply subboth. apply IHsubseq.
  - simpl. apply subext. apply IHsubseq.
Qed.
...
Proof. 
  ...
  - intros. inversion H. apply subnil.
  - intros. inversion H.
    + apply subnil.
    + apply subboth. apply IHsubseq. apply H3.
    + apply subext. apply IHsubseq. apply H3.
  - intros. inversion H.
    + apply subnil.
    + rewrite H2. apply subext. apply IHsubseq. apply H.
    + apply subext. apply IHsubseq. apply H.
Qed.
(** [] *)
...
(* ref: https://github.com/duinomaker/LearningStuff/blob/8c2b985ea73e0b77cae5fff8feb9747708a327a3/LogicalFoundations/ch7-regex_refl.v *)
...
Proof.
  ...
  - generalize dependent s2. induction s1.
    + intros. induction s2. simpl. apply MEmpty. inversion H.
    + intros. inversion H. simpl.
      assert (H1: x :: s1 = [x] ++ s1). reflexivity.
      rewrite H1. apply MApp. apply MChar. apply IHs1. reflexivity.
Qed.
(** [] *)
...
Lemma MStar'' : forall T (s : list T) (re : reg_exp T),
  ...
  exists ss : list (list T),
    ...
    /\ forall s', In s' ss -> s' =~ re.
Proof.
  intros. remember (Star re) as reeq.
  induction H.
  - discriminate.
  - discriminate.
  - discriminate.
  ...
  - exists [s1;s2].
    assert (Heqreeq' := Heqreeq). 
    (* assert (Hre0: re0 = Star re). rewrite <- Heqreeq. apply MStarApp.  *)
    apply IHexp_match2 in Heqreeq'. destruct Heqreeq'. injection Heqreeq. intros. rewrite <- H2. split.
    + unfold fold. rewrite app_nil_r. reflexivity.
    + simpl. intros. destruct H3. destruct H1.
      * rewrite H2. rewrite <- H3. rewrite <- H2. apply H. 
      * destruct H3. rewrite <- H3. rewrite H2.
     assert (Hre0: re0 = Star re).
...
Proof.
  ...
  - simpl in *. intros. rewrite app_length in H. apply add_le_cases in H. destruct H.
    + apply IH1 in H. destruct H as [s3 [s4 [s5 H]]].
      exists (s3), (s4), (s5 ++ s2). destruct H as [H1 [H2 H3]]. split.
      * rewrite H1. rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
      * split. apply H2. intros. apply MApp. 
      (* TLDW *)
      (* see https://github.com/duinomaker/LearningStuff/blob/8c2b985ea73e0b77cae5fff8feb9747708a327a3/LogicalFoundations/ch7-regex_refl.v#L381 *)
  (* FILL IN HERE *) Admitted.
(** [] *)
...
Proof.
  intros. inversion H.
  - split. intros. reflexivity. intros. apply H0.
  - split. intros. apply H0 in H2. destruct H2. discriminate.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. destruct (eqbP n x).
    + unfold not. simpl. intros. apply IHl. inversion H0. destruct H1.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  ...
  | nost_mult x1 x2 l (H1: nostutter (x2::l)) (H2: x1 <> x2): nostutter (x1::(x2::l))
.
...
Inductive in_order_merge {X: Type} : list X -> list X -> list X -> Prop :=
  ...
  | iom_add_l1 x1 l1 l2 l (H: in_order_merge l1 l2 l) : in_order_merge (x1::l1) l2 (x1::l)
  | iom_add_l2 x2 l1 l2 l (H: in_order_merge l1 l2 l) : in_order_merge l1 (x2::l2) (x2::l)
.

Lemma filter_twice_refl: forall (X: Type) (l: list X) test, filter test l = filter test (filter test l).
Proof.
  intros. induction l.
  - simpl. reflexivity.
  - destruct (test x) eqn: eqx.
    + simpl. rewrite eqx. simpl. rewrite eqx. rewrite <- IHl. reflexivity.
    + simpl. rewrite eqx. rewrite <- IHl. reflexivity.
Qed.

Lemma cons_false: forall (X: Type) (l: list X) x, ~ (l = x :: l).
Proof.
