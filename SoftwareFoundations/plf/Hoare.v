...
Theorem hoare_pre_false : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros. unfold hoare_triple. intros. exfalso. unfold not in H. apply H with st. assumption.
Qed.
(** [] *)
...
(* FILL IN HERE *)
(* a = 2 * X, with X <> 0 as the initial state *)
...
Proof.
  ...
  replace (X !-> st X; X !-> aeval st a; st) with st. split.
  - assumption.
  - unfold t_update. Search (eqb_string).
    rewrite <- eqb_string_refl. reflexivity.
  - symmetry. unfold t_update. apply functional_extensionality.
    intros. destruct (eqb_string X x) eqn: eqXx.
    Search (eqb_string). apply eqb_string_true_iff in eqXx.
...
Proof.
  ...
  apply hoare_asgn_fwd with a st. assumption. split. split. assumption.
  auto. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - apply hoare_asgn.
  - unfold "->>". unfold assn_sub, t_update.
...
Proof.
  unfold hoare_triple.
  intros H.
  specialize H with (a := X) (n := 0) (st := empty_st) (st' := ( Y!-> 3; X !-> 3)). simpl in H.
  ...
  apply H in H0. unfold t_update in H0. simpl in H0. discriminate H0.
  reflexivity.
Qed.
(** [] *)
...
(*     inversion H0. unfold t_update. simpl. inversion H1. reflexivity. *)
(*   - unfold hoare_triple. intros. inversion H. inversion H0. subst. inversion H1. simpl in H4. *)
(*     Search (negb _ <> true). apply eq_true_negb_classical in H4. *)
(*     Search ( _ =? _ = true). apply beq_nat_true in H4. rewrite H4 in H3. inversion H3. simpl. lia. *)
(* Qed. *)
...
Proof.
  unfold ex1_repeat. eapply E_RepeatTrue.
  - eapply E_Seq. constructor. eauto. constructor. eauto.
  - reflexivity.
Qed.
...
Proof.
  unfold hoare_triple. intros.
  inversion H. subst. unfold havoc_pre in *. auto.
Qed.
...
Proof.
  ...
  - apply hoare_havoc.
  - unfold "->>", havoc_pre, assn_sub. intros. exists (st X).
    Print t_update_shadow. rewrite t_update_shadow. simpl.
    Search (t_update). rewrite t_update_same. assumption.
Qed.
...
Theorem assert_assume_differ : exists (P:Assertion) b (Q:Assertion),
  ...
  /\ ~ ({{P}} assert b {{Q}}).
Proof.
  exists True, (BEq X 1), True. split.
  - unfold hoare_triple. intros.
    inversion H. subst. simpl in *. exists st. split. auto. auto.
...
Proof.
  ...
  apply H; try apply E_AssertTrue; assumption.
Qed.  
...
Proof.
  ...
  - destruct H0. auto.
  - inversion H0. inversion H3. rewrite H2 in H5. discriminate H5.
Qed.
...
Theorem hoare_assume : forall Q b: bexp,
    {{ Q }} assume b {{Q /\ b}}.
Proof.
  unfold hoare_triple. intros.
...
Proof.
  ...
  - eapply hoare_consequence_pre. eapply hoare_consequence_post.
    apply (hoare_assert BTrue). auto.
    unfold "->>". intros. inversion H. split. simpl. reflexivity. simpl. inversion H0. rewrite H3. auto.
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold "->>", assn_sub, t_update. intros. simpl. inversion H. inversion H0. split.
    lia. auto.
  - eapply hoare_consequence_pre. eapply hoare_consequence_post.
    apply (hoare_assume BTrue).
    unfold "->>". intros. inversion H. inversion H1. split.
    Search ( _ =? _ = true). apply beq_nat_true in H3. simpl. auto. auto.
    unfold "->>". simpl. auto.
Qed.
